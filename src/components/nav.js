  ...
  return (
    <nav className="navbar fixed top-0 left-0 right-0 z-100 flex justify-between items-center bg-[#021838fa] p-5 overflow-hidden w-screen" id="navbar">
      <div className="left">
        <a href="#">
          <div
            className={`logo h-[30px] w-[90px] my-auto mt-2 ml-4 transition-transform duration-700 ease-in-out delay-200${
              logoVisible ? 'transform translate-x-0' : 'transform -translate-x-full'
            }`}
          >
      ...
      {/* Hamburger button to toggle .right div */}
      <button className="ham" onClick={handleToggle}>
        <i className="fa-solid fa-bars"></i>
      </button>

      <div className={`right ${navVisible ? 'show' : ''}`}>
